...
Today, we will be looking at real data from Public Health Scotland - the dataset is called `Week6_unintentional_injuries.csv` and contains data from the 9 NHS Health Boards in Scotland about emergency hospital admissions as a result of unintentional injuries and assaults.
...
injuries_admissions <- read_csv(here("week_6", "data", "Week6_unintentional_injuries.csv"))
...
**First** Load the NHS Health board Shapefile (Week6_NHS_healthboards_2019.shp) and save it into an object called `NHS_healthboards` using `st_read`. Examine the `NHS_healthboards` object. What is the geometry type?
...
NHS_healthboards <- st_read(here("week_6", "data", "Week6_NHS_HealthBoards_2019.shp"))
...
You will need to read in the `Week6_hb2019_pop_est.csv` which is a dataset of population estimates for all 14 health boards (2019) in Scotland from 1981 to 2022. The data dictionary link is here:
...
hb_populations_female <- read_csv(here("week_6", "data", "Week6_hb2019_pop_est.csv")) %>% 
  filter(
    Year == 2022,
    Sex == "Female",
    HB != "S92000003"
...
f75plus_falls_per_10k_per_hb <- NHS_healthboards %>%
  full_join(f75plus_falls_per_10k, 
            by = join_by(HBCode == HB)) 
