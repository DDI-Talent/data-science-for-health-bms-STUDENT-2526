...
Our task today be to plot these aggregated data in the form of a **map** of NHS boards, to gain better insights into unintentional injury admissions across Scotland. Aggregated data means that there are recorded observations of every combination of variable category.
...
**First** Load the NHS Health board Shapefile (Week6_NHS_healthboards_2019.shp) and save it into an object called `NHS_healthboards` using `st_read` from the `sf` package. Examine the `NHS_healthboards` object. What is the geometry type?
